    ...
    if balanced:
        ...
        for entity_type in ENTITY_TYPES:
            ...
            while count < samples_per_entity and attempts < samples_per_entity * 5:
                ...
                # Apply Noise
                if noise_level > 0:
                    # Note: If we noise the text, we might break exact string matching for entities.
                    # However, align_labels in run_ner_bilstm_crf.py uses text.find(entity_value).
                    # If entity_value is clean but text is noisy, find() might fail or find wrong place.
                    # Ideally we should noise the text BUT keep track of entity indices.
                    # Since our current pipeline relies on string matching, we should be careful.
                    # For now, let's apply noise ONLY if we can ensure entities are preserved or also noised?
                    # Or, we can just apply noise to the parts of text that are NOT entities?
                    # That's hard with simple string replacement.
                    
                    # Alternative: Apply noise to the whole text, and accept that some entities might be missed during labeling
                    # (which is actually good for robustness - "missing label" noise).
                    # But we want to train the model to recover them.
                    # If the label is missing in training data, the model learns to ignore them. That's bad.
                    
                    # So, we will NOT apply character noise that destroys entities for now, 
                    # unless we update the entity list to match the noised text.
                    # But we CAN apply space noise (which align_labels handles by skipping whitespace? 
                    # No, align_labels splits by whitespace).
                    # If we insert space inside a token, it splits into two tokens.
                    # e.g. "홍길동" -> "홍 길동". Labeler will see "홍", "길동".
                    # If entity is "홍길동", text.find("홍길동") will fail.
                    
                    # Safe noise: Add noise to non-entity parts?
                    # Or just skip noise for this iteration to ensure high quality labels, 
                    # and rely on the "Negative Samples" and "Template Diversity" for robustness.
                    pass
