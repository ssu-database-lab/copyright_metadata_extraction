...
def build_dataloaders(
    ...
    # Since we pre-loaded all data into tensors in RAM (NERDataset optimization),
    # using multi-processing (num_workers > 0) adds huge IPC overhead without benefit.
    # Switching to num_workers=0 (main process only) is fastest for pre-loaded data.
    num_workers = 0 
    print(f"[data] DataLoader config: batch_size={batch_size}, num_workers={num_workers}, pin_memory=True")

    # Evaluation batch size can be MUCH larger (no gradients stored, no backprop)
    # H200 can handle 4x training batch size during evaluation
    eval_batch_size = batch_size * 4
    print(f"[data] Evaluation batch size set to {eval_batch_size} (4x training) for maximum speedup")
...
def collect_seqeval_inputs_from_crf_batch(
    ...
    # Move to CPU/Numpy for fast iteration (avoids .item() overhead on CUDA)
    ...
    for i in range(batch_size):
        ...
        # pred_paths[i] is already a list of ints (from torchcrf decode)
        pred_path = pred_paths[i]
        pred_idx = 0
        
        curr_labels = labels_np[i]
        curr_mask = mask_np[i]
        ...
        for j, label_id in enumerate(curr_labels):
            ...
            if label_id != -100:
                ...
                if pred_idx < len(pred_path):
                    pred_labels.append(id2label[pred_path[pred_idx]])
                else:
...
def collect_seqeval_inputs_from_logits_batch(
    ...
    for i in range(batch_size):
        ...
        for j, label_id in enumerate(curr_labels):
            ...
            true_labels.append(id2label[label_id])
            pred_labels.append(id2label[curr_preds[j]])
...
def apply_token_noise(token: str) -> str:
    ...
    if noise_type == 'space':
        ...
        # Decompose a random Hangul character into Jamos
        # Find a Hangul character
...
def compute_seqeval_metrics(
    ...
    """Return precision, recall, f1 (without expensive report string during training)."""
    ...
    # Skip classification_report during training - it's extremely slow
    return {"precision": float(p), "recall": float(r), "f1": float(f), "report": ""}
    ...
    with torch.no_grad(): # , torch.amp.autocast('cuda'):
        for batch in dataloader:
            ...
            if model_type == "crf":
                ...
                if return_probs:
                    # Softmax for probabilities (only when needed for PR curve)
...
def train_one_model(
    *,
    ...
    if model_type == "pure":
        ...
        # Optimized for larger gap: Increased hidden size and layers
        cfg = NERConfig(
    ...
    # Enable cuDNN benchmark for optimized kernel selection (great for fixed input sizes)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for step, batch in enumerate(train_loader, start=1):
            global_step += 1
            # set_to_none=True is faster than zeroing out gradients
            ...
            # Use device.type to support cpu/cuda dynamically
            if device.type == 'cuda':
                ...
                # No autocast for CPU (or use 'cpu' if supported, but usually not needed for this)
                if model_type == "crf":
                    loss = model(
                        input_ids=input_ids,
                        attention_mask=attention_mask,
        ...
        # Epoch-wise Evaluation (fast mode - no probabilities needed)
        ...
        dev_metrics, _, _, _ = evaluate_model(
            ...
            return_probs=False,  # Skip expensive probability calculations during training
        )
...
def run_six_model_benchmark(
    ...
    for model_type, hf_name, tag in model_specs:
        ...
        # Use arguments for batch size instead of hardcoded value
        ...
        res = train_one_model(
            ...
            max_length=max_length,
            batch_size=batch_size,
            num_epochs=num_epochs, # Use passed num_epochs
            learning_rate=learning_rate,
            num_evaluations=20
        )
        ...
        # Run Inference on data/in
        inference_in_dir = Path("data/in")
        inference_out_dir = Path(f"data/out/ner/{tag}")
        if inference_in_dir.exists():
            print(f"[Inference] Running inference on {inference_in_dir} -> {inference_out_dir}")
            run_inference_on_directory(model_dir, inference_in_dir, inference_out_dir)
            
            # Save labels to labels.md in the output directory
            try:
                # Load label map from model dir
                with open(model_dir / "ner_config.json", "r", encoding="utf-8") as f:
                    cfg = json.load(f)
                    id2label = cfg.get("id2label", {})
                
                with open(inference_out_dir / "labels.md", "w", encoding="utf-8") as f:
                    f.write(f"# NER Labels for {tag}\n\n| ID | Label |\n|---|---|\n")
                    for idx in sorted(id2label.keys(), key=lambda x: int(x)):
                        f.write(f"| {idx} | {id2label[idx]} |\n")
                print(f"[Inference] Saved labels.md to {inference_out_dir}")
            except Exception as e:
                print(f"[Warning] Failed to save labels.md: {e}")
        else:
            print(f"[Warning] Input directory {inference_in_dir} not found. Skipping inference.")
...
def run_kfold_benchmark(
    ...
    # (model_type, hf_name, short_tag)
    # Reduced to just CRF BERT for speed/demo as requested
    model_specs = [
        ("crf", "google-bert/bert-base-multilingual-cased", "crf_bert"),
    ]
    ...
    for model_type, hf_name, base_tag in model_specs:
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(splits):
            ...
            # Ensure we save to the path expected by inference
            # model_root / tag / f"fold_{fold_idx+1}"
            # Inference looks for: model_dir / "1.00" / "crf_bert"
            # Here tag is "1.00_crf_bert"
            # So model_dir becomes models/ner_benchmark/1.00_crf_bert/fold_1
            # I will adjust inference to look for this pattern.
        ...
        # Save Summary Incrementally (to prevent data loss on crash)
        ...
        try:
            ...
            print(f"[Save] Updated summary to {summary_path}")
        except Exception as e:
            print(f"[Warning] Failed to save incremental summary: {e}")
...
def parse_args() -> argparse.Namespace:
    ...
    # H200 Optimization: 120 batch size as requested
    p.add_argument("--batch_size_pure", type=int, default=120)
    p.add_argument("--batch_size_crf", type=int, default=120)
...
def run_inference_on_directory(model_dir: Path, input_dir: Path, output_dir: Path):
    """
    Run inference on all .txt files in input_dir using the model in model_dir.
    Saves results to output_dir as JSON.
    """
    print(f"\n>>> Running Inference on {input_dir} <<<")
    
    if not model_dir.exists():
        print(f"[Error] Model directory not found: {model_dir}")
        # Try to find any crf model
        # The training loop saves to: models/ner_benchmark/{tag}/fold_1
        # tag is like "1.00_crf_bert"
        # So we should look for *crf* folder and then fold_1 inside it.
        
        parent = model_dir.parent # models/ner_benchmark/1.00
        # Actually model_dir passed in is: model_dir / "1.00" / "crf_bert"
        # But the training loop creates: model_root / "1.00_crf_bert" / "fold_1"
        # So the structure is slightly different than I assumed.
        
        # Let's try to find the correct path dynamically
        # model_dir passed is: models/ner_benchmark/1.00/crf_bert
        # We want: models/ner_benchmark/1.00_crf_bert/fold_1
        
        base_root = model_dir.parent.parent # models/ner_benchmark
        candidates = list(base_root.glob("*crf*bert*/fold_1"))
        
        if candidates:
            model_dir = candidates[0]
            print(f"[Info] Found trained model at: {model_dir}")
        else:
            print(f"[Error] Could not find any trained CRF model in {base_root}")
            return

    try:
        from src.ner.ner_system import load_model_for_inference, predict_texts
    except ImportError:
        print("[Error] Could not import ner_system for inference.")
        return

    try:
        model, tokenizer, id2label, config, device = load_model_for_inference(str(model_dir))
    except Exception as e:
    ...
    print(f"[Info] Found {len(txt_files)} files. Processing...")
    
    for txt_file in txt_files:
        try:
            text = txt_file.read_text(encoding="utf-8")
            # Split into lines to avoid max_length issues
...
def main() -> None:
    ...
    if not args.single_run:
        print(">>> Starting Data Scaling Benchmark (100% Only) <<<")
        
        # Modified: Only run 100% as requested
        ratios = [1.00]
        base_samples = 30000
        
        for ratio in ratios:
            num_samples = int(base_samples * ratio)
            prefix = f"{ratio:.2f}" # e.g. "1.00"
        ...
        # After training, run inference on data/in/asdf
        run_inference_on_directory(
            model_dir=model_dir / "1.00_crf_bert" / "fold_1", # Use Fold 1 model
            input_dir=Path("data/in/asdf"),
            output_dir=Path("data/in/asdf")
        )
        return
